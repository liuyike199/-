...
namespace CoreFrame.Controllers
{
	/// <summary>
	/// 支付宝当面付款
	/// </summary>
    public class FTFPayController : Controller
    {
		...
		public FTFPayController(IWebHostEnvironment hostingEnvironment, IAlipayF2FService alipayF2FService, IAlipayService alipayService)
		{
		...
		private AlipayTradePrecreateContentBuilder BuildPrecreateContent(string orderName, string orderAmount, string outTradeNo)
		{
			...
			//收款账号
			builder.seller_id = _alipayService.Options.Uid; //_alipayService.Options.Uid;
			//订单编号
				...
				using (QRCodeData qrCodeData = qrGenerator.CreateQrCode(str, eccLevel))
				{
					using (QRCode qrCode = new QRCode(qrCodeData))
					{
						...
						Bitmap bp = qrCode.GetGraphic(20, Color.Black, Color.White,
							new Bitmap(Path.Combine(_hostingEnvironment.WebRootPath, "FTFPay/images/alipay.png")), 15);
						return bp;
					}
				}
			}
